...
namespace chen {
	...
	bool ProxyVideoTrackSource::OnFrameTexture(void *texture, uint32 fmt, int32_t width, int32_t height)
	{
		...
		} else if (i420_buffer_->width() * i420_buffer_->height() <
			   width * height) {
			i420_buffer_ = webrtc::I420Buffer::Create(width, height);
		}
		//i420_buffer_->set_texture(texture);
		//	g_dxgi_format = static_cast<DXGI_FORMAT>(fmt);
		NORMAL_EX_LOG("[fmt = %u][width = %u][height = %u]", fmt, width,
			      height);
	...
	bool ProxyVideoTrackSource::OnFrame(unsigned char *rgba, uint32 fmt,
					  int32_t width,
		     int32_t height)
	{
		if (!rgba) {
		...
		if (width < 1 || height < 1) {
			WARNING_EX_LOG("osg  width = %d, height = %d ", width,
				       height);
			return false;
		}
		...
		NORMAL_EX_LOG("[fmt = %u][width = %u][height = %u]", fmt, width,
			      height);
		::memcpy(i420_buffer_->MutableDataY(), rgba,
			 width * height * 4);
		//libyuv::ConvertToI420(rgba, 0, i420_buffer_->MutableDataY(),
		//	i420_buffer_->StrideY(), i420_buffer_->MutableDataU(),
		//	i420_buffer_->StrideU(), i420_buffer_->MutableDataV(),
	...
	bool ProxyVideoTrackSource::OnFrame(unsigned char *y_ptr, uint32 fmt,
		...
		::memcpy(i420_buffer_->MutableDataU(), uv_ptr,
			 width * height / 2);
		//NORMAL_EX_LOG("");
		//libyuv::ConvertToI420(rgba, 0, i420_buffer_->MutableDataY(),
		//	i420_buffer_->StrideY(), i420_buffer_->MutableDataU(),
