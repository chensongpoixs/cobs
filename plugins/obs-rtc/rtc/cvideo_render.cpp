...
static recv_video_callback video_callback_ptr = NULL;
	...
	{
		{
			...
			image_.reset(new uint8_t[size]);
			//static uint8 *rgba_ptr = new uint8[];
			...
			/*NORMAL_EX_LOG("[width = %u][height = %u]",
				      buffer->width(), buffer->height());*/
			if (video_callback_ptr)
			{
				video_callback_ptr(
					data_ptr,
						   (unsigned char *)image_.get(),
						   buffer->width(), buffer->height());
			}
...
void cpp_set_recv_video_callback(void *data, recv_video_callback callback)
{
	//cpp_set_recv_video_callback(callback);
	data_ptr = data;
	video_callback_ptr = callback;
}

void c_set_recv_video_callback(void *data, recv_video_callback callback)
{
	cpp_set_recv_video_callback(data, callback);
}
