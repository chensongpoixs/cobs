...
namespace chen 
{
	...
	bool crtc_player::send_offer(const std::string & offer)
	{
		...
		nlohmann::json data = {
			...
			{"video_peerid", g_cfg.get_string(ECI_UserName)},
	...
	std::thread g_thread;
static void g_rtc_thread()
{
	chen::g_rtc_player.Loop();
}
bool   c_player_rtc_init()
{
	const char *config_file = "./client.cfg";
	return chen::g_rtc_player.Init(config_file);
}
void   c_player_rtc_startup()
{
	std::thread td(&g_rtc_thread);
	g_thread.swap(td);
}
void   c_player_rtc_destroy()
{
...
bool   cpp_player_rtc_init()
{
	return c_player_rtc_init();
}
void cpp_player_rtc_startup()
{
	c_player_rtc_startup();
}
void cpp_player_rtc_destroy()
{
