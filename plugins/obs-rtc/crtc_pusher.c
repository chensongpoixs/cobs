...
{
	struct rtc_pusher_capture *wc = data;
	if (!wc)
	{
...
static void log_settings(struct rtc_pusher_capture *wc, obs_data_t *s)
{
...
static void update_settings(struct rtc_pusher_capture *wc, obs_data_t *s)
{
...
static const char *rtc_pusher_getname(void *unused)
{
...
static void *rtc_pusher_create(obs_data_t *settings, obs_source_t *source)
{
	struct rtc_pusher_capture *wc =
		bzalloc(sizeof(struct rtc_pusher_capture));
...
static void rtc_pusher_actual_destroy(void *data)
{
	struct rtc_pusher_capture *wc = data;
...
static void rtc_pusher_destroy(void *data)
{
	c_capture_destroy();
	c_rtc_destroy();
	obs_queue_task(OBS_TASK_GRAPHICS, rtc_pusher_actual_destroy, data,
		       false);
}

static void force_reset(struct rtc_pusher_capture *wc)
{
	wc->window = NULL;
...
static void rtc_pusher_update(void *data, obs_data_t *settings)
{
	struct rtc_pusher_capture *wc = data;
	update_settings(wc, settings);
	log_settings(wc, settings);
...
static uint32_t rtc_pusher_width(void *data)
{
	struct rtc_pusher_capture *wc = data;
...
static uint32_t rtc_pusher_height(void *data)
{
	struct rtc_pusher_capture *wc = data;
...
static void rtc_pusher_defaults(obs_data_t *defaults)
{
...
static bool rtc_pusher_capture_method_changed(obs_properties_t *props,
				      obs_property_t *p, obs_data_t *settings)
{
	UNUSED_PARAMETER(p);

	struct rtc_pusher_capture *wc = obs_properties_get_param(props);
	if (!wc)
		return false;
...
static bool rtc_pusher_window_changed(obs_properties_t *props,
				      obs_property_t *p,
			      obs_data_t *settings)
{
	struct rtc_pusher_capture *wc = obs_properties_get_param(props);
...
static obs_properties_t *rtc_pusher_properties(void *data)
{
	struct rtc_pusher_capture *wc = data;
	...
	obs_property_set_modified_callback(p, rtc_pusher_window_changed);
	...
	obs_property_set_modified_callback(p,
					   rtc_pusher_capture_method_changed);
...
static void rtc_pusher_hide(void *data)
{
	struct rtc_pusher_capture *wc = data;
...
static void rtc_pusher_tick(void *data, float seconds)
{
	struct rtc_pusher_capture *wc = data;
	RECT rect;
...
static void rtc_pusher_render(void *data, gs_effect_t *effect)
{
	struct rtc_pusher_capture *wc = data;
...
struct obs_source_info rtc_pusher_info = {
	...
	.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_CUSTOM_DRAW |
			OBS_SOURCE_SRGB,
	.get_name = rtc_pusher_getname,
	.create = rtc_pusher_create,
	.destroy = rtc_pusher_destroy,
	.update = rtc_pusher_update,
	.video_render = rtc_pusher_render,
	.hide = rtc_pusher_hide,
	.video_tick = rtc_pusher_tick,
	.get_width = rtc_pusher_width,
	.get_height = rtc_pusher_height,
	.get_defaults = rtc_pusher_defaults,
	.get_properties = rtc_pusher_properties,
